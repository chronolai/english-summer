...
const ianRound3 = [{
  ...
  date: '2021-09-07',
  rows: [135, 467, 639, 441, 600, 15, 1097, 438, 452, 606].map(id => `basic1200_${id}`),
}, {
  date: '2021-09-08',
  rows: [204, 655, 1046, 82, 1146, 771, 810, 517, 750, 205].map(id => `basic1200_${id}`),
}, {
  date: '2021-09-09',
  rows: [728, 560, 105, 493, 571, 671, 605, 680, 1088, 744].map(id => `basic1200_${id}`),
  // }, { +3 天
  //   date: '2021-09-10',
  ...
  // }, { +3 天
  //   date: '2021-09-14',
  //   rows: [584, 321, 410, 397, 306, 823, 503, 283, 631, 624].map(id => `basic1200_${id}`),
}, {
  date: '2021-09-15',
  rows: [584, 321, 410, 397, 306, 823, 503, 283, 631, 624].map(id => `basic1200_${id}`),
}, {
  date: '2021-09-16',
  rows: [825, 1076, 968, 932, 609, 618, 254, 213, 898, 172].map(id => `basic1200_${id}`),
}, {
  date: '2021-09-17',
  rows: [].map(id => `basic1200_${id}`),
}, {
...
const doraRound3 = [{
  ...
  date: '2021-09-07',
  rows: [
    ...[32, 111, 566, 826, 650].map(id => `basic1200_${id}`),
    ...[680, 429, 565, 354, 366, 239, 122, 640, 480, 225, 90, 777, 238, 109, 326].map(id => `advance800_${id}`),
  ],
}, {
  date: '2021-09-08',
  rows: [
    ...[431, 422, 536, 1159, 401].map(id => `basic1200_${id}`),
    ...[607, 734, 123, 551, 38, 153, 66, 597, 519, 29, 340, 595, 388, 703, 570].map(id => `advance800_${id}`),
  ],
}, {
  date: '2021-09-09',
  rows: [
    ...[744, 458, 282, 70, 1110].map(id => `basic1200_${id}`),
    ...[416, 604, 248, 338, 668, 315, 389, 547, 8, 442, 492, 241, 709, 694, 314].map(id => `advance800_${id}`),
  ],
}, {
  date: '2021-09-10',
  rows: [
    ...[94, 882, 902, 984, 180].map(id => `basic1200_${id}`),
    ...[430, 471, 567, 455, 638, 496, 51, 704, 214, 210, 688, 374, 660, 639, 310].map(id => `advance800_${id}`),
  ],
}, {
  date: '2021-09-13',
  rows: [
    ...[7, 841, 541, 801, 275].map(id => `basic1200_${id}`),
    ...[104, 417, 679, 262, 316, 731, 634, 302, 202, 434, 146, 460, 92, 784, 692].map(id => `advance800_${id}`),
  ],
}, {
  date: '2021-09-14',
  rows: [
    ...[645, 385, 1104, 1111, 1164].map(id => `basic1200_${id}`),
    ...[454, 278, 273, 758, 363, 426, 259, 771, 577, 189, 160, 778, 658, 386, 403].map(id => `advance800_${id}`),
  ],
}, {
  date: '2021-09-15',
  rows: [
    ...[437, 308, 134, 945, 261].map(id => `basic1200_${id}`),
    ...[667, 283, 93, 735, 459, 642, 711, 637, 203, 332, 28, 350, 420, 360, 486].map(id => `advance800_${id}`),
  ],
}, {
  date: '2021-09-16',
  rows: [
    ...[140, 60, 430, 1053, 202].map(id => `basic1200_${id}`),
    ...[46, 578, 456, 677, 487, 749, 328, 559, 542, 12, 516, 631, 184, 463, 414].map(id => `advance800_${id}`),
  ],
}, {
  date: '2021-09-17',
  rows: [
    ...[518, 660, 462, 835, 548].map(id => `basic1200_${id}`),
    ...[201, 257, 600, 200, 2, 174, 125, 1, 68, 579, 291, 723].map(id => `advance800_${id}`),
  ],
}, {
  date: '2021-09-24',
  rows: [
    ...[703, 755, 233, 808, 98].map(id => `basic1200_${id}`),
    ...[2517, 7705, 2949, 2935, 5896, 2093, 676, 4346, 2083, 8635, 6564, 7352, 7853, 2768, 8621].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-09-27',
  rows: [
    ...[147, 775, 150, 426, 452].map(id => `basic1200_${id}`),
    ...[499, 7791, 8653, 542, 206, 3476, 289, 2665, 5134, 1929, 4835, 2684, 2830, 1531, 5442].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-09-28',
  rows: [
    ...[805, 1057, 444, 51, 1059].map(id => `basic1200_${id}`),
    ...[6085, 8347, 1080, 44, 5878, 3139, 8136, 8428, 7442, 7939, 2345, 961, 2381, 2987, 7336].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-09-29',
  rows: [
    ...[685, 1090, 346, 139, 4].map(id => `basic1200_${id}`),
    ...[6784, 3892, 4993, 8290, 2110, 6543, 868, 6530, 2384, 8074, 7067, 1930, 6307, 3323, 5083].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-09-30',
  rows: [
    ...[591, 63, 135, 314, 33].map(id => `basic1200_${id}`),
    ...[5075, 5553, 7570, 329, 2965, 1688, 8693, 1348, 8425, 6725, 2990, 534, 538, 1943, 3343].map(id => `senior8000_${id}`),
  ],
}].map(mapFunc('dora', 3));
...
const doraRound4 = [{
  date: '2021-10-13',
  rows: [
    ...[338, 1048, 950, 558, 3].map(id => `basic1200_${id}`),
    ...[7229, 6979, 537, 4176, 3772, 2612, 2851, 692, 5352, 8283, 4597, 8184, 4168, 1486, 4588].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-10-14',
  rows: [
    ...[315, 698, 1085, 443, 8].map(id => `basic1200_${id}`),
    ...[2529, 4697, 8017, 2116, 6308, 7881, 696, 5648, 3604, 7603, 4657, 54, 4666, 7967, 2061].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-10-15',
  rows: [
    ...[107, 779, 1006, 122, 894].map(id => `basic1200_${id}`),
    ...[7966, 6199, 6242, 6948, 8391, 6727, 5099, 5533, 873, 4629, 7335, 6805, 4799, 8278, 3239].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-10-18',
  rows: [
    ...[332, 1035, 336, 1007, 845].map(id => `basic1200_${id}`),
    ...[2382, 6330, 3002, 4556, 7051, 372, 5378, 5366, 4179, 7285, 417, 5706, 5228, 2502, 7858].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-10-19',
  rows: [
    ...[200, 58, 592, 1170, 1146].map(id => `basic1200_${id}`),
    ...[3606, 6548, 4411, 1091, 1249, 45, 631, 74, 8272, 6050, 4051, 2944, 663, 5202, 6388].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-10-20',
  rows: [
    ...[303, 417, 104, 642, 1087].map(id => `basic1200_${id}`),
    ...[3454, 6061, 6510, 5652, 3899, 6400, 1111, 3751, 7146, 7241, 4436, 2743, 5797, 132, 6086].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-10-22',
  rows: [
    ...[1136, 741, 595, 380, 696].map(id => `basic1200_${id}`),
    ...[7330, 3080, 6216, 1041, 5832, 2579, 8424, 2070, 6550, 2256, 2954, 8761, 3074, 1551, 2868].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-10-26',
  rows: [
    ...[774, 1139, 1129, 149, 291].map(id => `basic1200_${id}`),
    ...[6271, 2018, 7143, 6212, 5210, 7452, 1937, 6003, 5717, 5258, 3838, 904, 8701, 7140, 3815].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-10-27',
  rows: [
    ...[1025, 165, 496, 225, 818].map(id => `basic1200_${id}`),
    ...[7014, 6375, 8533, 7474, 835, 6756, 3690, 4298, 5583, 4395, 3028, 7915, 4464, 2158, 2897].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-10-28',
  rows: [
    ...[227, 860, 1083, 506, 1086].map(id => `basic1200_${id}`),
    ...[1682, 4044, 5983, 7057, 4611, 3578, 8289, 7042, 4864, 7036, 6688, 2896, 4415, 2664, 8622].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-11-01',
  rows: [
    ...[1094, 325, 900, 809, 438].map(id => `basic1200_${id}`),
    ...[5221, 583, 7593, 3871, 7731, 348, 6840, 3135, 6082, 2078, 7740, 5379, 5712, 7965, 6586].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-11-02',
  rows: [
    ...[1130, 290, 453, 402, 977].map(id => `basic1200_${id}`),
    ...[5084, 4921, 3240, 4483, 7003, 2623, 3027, 4222, 2044, 3401, 6778, 4353, 5609, 3016, 815].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-11-03',
  rows: [
    ...[118, 909, 57, 450, 382].map(id => `basic1200_${id}`),
    ...[6918, 7846, 3423, 2099, 3829, 7723, 3784, 6890, 8043, 6064, 5306, 565, 1020, 8713, 7018].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-11-04',
  rows: [
    ...[39, 105, 136, 416, 946].map(id => `basic1200_${id}`),
    ...[3253, 3504, 7374, 1166, 3556, 7657, 5198, 6598, 407, 3671, 7645, 6356, 1092, 6760, 2545].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-11-05',
  rows: [
    ...[274, 85, 341, 77, 1099].map(id => `basic1200_${id}`),
    ...[801, 7175, 2053, 418, 7861, 7234, 1131, 8778, 1178, 5548, 7236, 2669, 3808, 5074, 6990].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-11-08',
  rows: [
    ...[846, 933, 1175, 171, 643].map(id => `basic1200_${id}`),
    ...[5358, 2605, 2921, 5569, 2687, 4294, 897, 7781, 4863, 7094, 1231, 3306, 4097, 8614, 4723].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-11-089',
  rows: [
    ...[473, 279, 220, 40, 623].map(id => `basic1200_${id}`),
    ...[6624, 5480, 8292, 655, 7224, 5941, 7109, 1288, 3623, 6880, 7543, 8745, 1593, 4659, 6205].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-11-10',
  rows: [
    ...[694, 559, 252, 651, 234].map(id => `basic1200_${id}`),
    ...[4218, 5881, 4988, 345, 6909, 3257, 3276, 8762, 6026, 5779, 1464, 3388, 173, 411, 2626].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-11-11',
  rows: [
    ...[258, 477, 272, 525, 20].map(id => `basic1200_${id}`),
    ...[3393, 1702, 2404, 1628, 2688, 969, 4974, 6343, 1422, 6116, 7607, 1796, 5408, 7639, 3748].map(id => `senior8000_${id}`),
  ],
}, {
  date: '2021-11-12',
  rows: [
    ...[216, 1020, 349, 484, 834].map(id => `basic1200_${id}`),
    ...[7002, 7074, 2319, 4078, 3700, 5261, 3399, 1903, 1419, 8798, 8062, 556, 3526, 3029, 4737].map(id => `senior8000_${id}`),
  ],
}].map(mapFunc('dora', 4));
